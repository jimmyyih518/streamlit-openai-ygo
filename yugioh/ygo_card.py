...
class card_yugioh:
    def __init__(self, card=None, save=True):
        self.card_template = Image.open('assets/monster_effect.jpg').convert("RGBA")
        self.card_data = card
        self._proc_image(save=save)
        
    def _proc_image(self, save=True, savefile='assets/ygocard.jpg'):
        ...
        if "Stars" in self.card_data and "Attack" in self.card_data and "Defense" in self.card_data:
            star_size = (35, 35)
            self.monster_star = Image.open('assets/monster_star.png').resize(star_size).convert("RGBA")
            self._add_monster_attribs(self.card_data['Stars'], self.card_data["Attack"], self.card_data["Defense"])
        if save:
            self._save_image()
        return
        
    def _download_image(self, savefile='assets/ygocard.jpg'):
        url = self.card_data['image_url']
        resp = requests.get(url)
        self.card_img = Image.open(BytesIO(resp.content))

    def _add_img_to_card(self, savefile='assets/ygocard.jpg'):
        img_box = (381, 384)
    ...
    def _add_title(self, title):
        ...
        for font_size in range(80, 0, -1):
            font = ImageFont.truetype('assets/ygo_title.ttf', font_size)
    ...
    def _add_description(self, description):
        ...
        draw = ImageDraw.Draw(self.card_template)
        font = ImageFont.truetype('assets/ygo_description.ttf', 15)
    ...
    def _add_monster_attribs(self, stars=None, attack=0, defense=0):
        ...
        draw = ImageDraw.Draw(self.card_template)
        font = ImageFont.truetype('assets/ygo_description.ttf', 21)
        draw.text((320,695), str(attack), (0,0,0), font=font)
        draw.text((430,695), str(defense), (0,0,0), font=font)
        return
    
    def _save_image(self, savefile='assets/ygocard.jpg'):
